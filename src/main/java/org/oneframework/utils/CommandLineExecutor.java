...
import org.oneframework.enums.Platform;
import org.springframework.lang.NonNull;

import static org.oneframework.utils.StringUtils.streamToString;
...
public class CommandLineExecutor {
    ...
    public static boolean killAppiumProcessesByDeviceId(@NonNull String deviceId) {
        System.out.println("Killing appium process for "+ deviceId);
        String cmd = String.format(
                "ps -ef | grep -v stf | grep -i 'appium' | grep -i 'node' | grep -i 'relaxed-security' | grep -i '%s' | grep -v grep | awk '{print $2}' | tr -s '\\n' ' ' | xargs kill -9",
                deviceId);
        CommandLineResponse commandLineResponse = exec(cmd);
        System.out.println(commandLineResponse.getStdOut()+"for device "+deviceId);
        int i = commandLineResponse.getExitCode();
        if(i == 0) {
            return true;
        }else {
            return false;
